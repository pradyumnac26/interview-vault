...
const fs = require('modern-tar/fs');
...
async function downloadRepository(url, cwd, { headers } = {}) {
  ...
  try {
    ...
    await promises$1.pipeline(response.body, stream);
    await promises$1.pipeline(
      node_fs.createReadStream(tarFile),
      node_zlib.createGunzip(),
      fs.unpackTar(cwd, {
        strip: 1
...
const nuxthub = definePreset({
  ...
  async setupNitro(nitroConfig, options) {
    ...
    if (!nuxt.options.dev && hubConfig.database?.applyMigrationsDuringBuild) {
      await promises.mkdir(pathe.resolve(nitroConfig.rootDir, hubConfig.dir, "database/queries"), { recursive: true });
      let i = 1;
      ...
      for (const dumpFile of dumpFiles) {
        await promises.writeFile(pathe.resolve(nitroConfig.rootDir, hubConfig.dir, "database/queries", dumpFile.file), dumpFile.content);
      }
